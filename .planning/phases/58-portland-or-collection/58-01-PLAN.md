...
In `backend/src/scripts/content-generation/generate-locale-questions.ts`, make TWO additions:

5a. Find the `supportedLocales` object and add `'portland-or': portlandOrConfig`. Import `portlandOrConfig` at the top with the other city locale imports.

5b. Find the `configKeys` array on line 117 (currently lists `['bloomingtonConfig', 'losAngelesConfig', 'fremontConfig', 'norwichConfig', 'cambridgeMaConfig', 'planoTxConfig']`) and add `'portlandOrConfig'` to the end of the array. This is required for locale resolution — omitting it will cause a runtime throw even if supportedLocales is correct.

(State locales are auto-discovered; city locales require both the supportedLocales entry AND the configKeys array entry. This is the city-locale registration step.)
...
# Confirm portlandOrConfig appears in both supportedLocales and configKeys
grep "portlandOrConfig" backend/src/scripts/content-generation/generate-locale-questions.ts
# Expected: at least 2 matches (import + supportedLocales entry + configKeys entry)

# Confirm TypeScript compiles cleanly
cd backend && npx tsc --noEmit 2>&1 | head -20
```
  </verify>
  <done>portland-or.ts locale config exists with portlandOrConfig exported, targetQuestions 90, 5 topic categories, 20 sourceUrls. buildPortlandVoiceGuidance() exists in system-prompt.ts and is dispatched for localeSlug === 'portland-or'. generate-locale-questions.ts has portlandOrConfig in both supportedLocales and configKeys and is not corrupted. TypeScript compiles with no new errors.</done>
</task>
...
4. `generate-locale-questions.ts` has `portlandOrConfig` in both `supportedLocales` and `configKeys`
5. Audit shows 80+ net questions, 25–30% expiring ratio
6. Banner image exists at `frontend/public/images/collections/portland-or.jpg`
...
- portlandOrConfig registered in both supportedLocales and configKeys in generate-locale-questions.ts
