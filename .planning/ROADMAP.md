...
**Success Criteria** (what must be TRUE):
1. ElectionsPage jurisdiction dropdown is populated from `GET /api/admin/collections` (not game/collections) — a collection with fewer than 50 active questions appears in the dropdown
2. `POST /election-races` returns a 400 with a descriptive error if `jurisdiction` does not match any `collections.name` in the DB — no silent acceptance of mismatched names
3. `POST /regenerate` accepts an optional `collectionSlug` in the request body; when provided it overrides the name-based lookup; a renamed collection can be regenerated by passing the current slug
