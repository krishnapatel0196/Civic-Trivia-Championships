...
export function ResultsScreen({
  ...
  const wagerDisplay = (() => {
    if (!result.wagerResult || result.wagerResult.wagerAmount === 0)
      return { label: '—', color: G.inkMuted };
    return {
      label: `${result.wagerResult.won ? '+' : '−'}${result.wagerResult.wagerAmount.toLocaleString()}`,
      color: result.wagerResult.won ? G.correct : G.incorrect,
    };
  })();
